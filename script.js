...
function checkResetLogic() {
    ...
    let lastDay = new Date(appData.lastActiveDay).toISOString().slice(0, 10);
...
function resetStopwatch() {
    ...
    let totalStudyTime = Number(time.hour)*60 + Number(timer.min);
...
importBtn.disabled = true;
    ...
    /**
     * Update the info bar with file details.
     */
...
async function importLocalStorage(file) {
    ...
    // 3. watermark check (VERY IMPORTANT)
    if (
    ...
    if (backup.data.playerData.totalXp < currentPlayerData.totalXp) {
    ...
    // 4. restore data into localStorage
    for (const key in backup.data) {
        setLocalStorage(key, backup.data[key]);
    }

    // 5. optional: re-sync appData after import
    let appData = getLocalStorage("appData");
