...
function createAboutPage() {
    ...
    console.log('📦 Found:', { toc: !!toc, stickyToggle: !!stickyToggle, parentLink: !!parentLink, subMenu: !!subMenu, sections: allSections.length });
    ...
    if (toc && stickyToggle) {
        ...
        console.log('📌 Sticky preference:', stickyPref);
        if (stickyPref === 'true') {
        ...
        stickyToggle.addEventListener('click', function(e) {
            ...
            console.log('📌 Sticky now:', isSticky);
        });
    } else {
    ...
    if (parentLink && subMenu) {
        console.log('✅ Found parent and submenu');

        // Initially expand if any sub‑link is active
        const hasActiveSub = subMenu.querySelector('.toc__sub-link.active');
        if (hasActiveSub) {
            parentLink.classList.add('open');
            subMenu.classList.add('open');
            console.log('📂 Submenu initially expanded');
        }
        ...
        parentLink.addEventListener('click', function(e) {
            ...
            e.stopPropagation();
            console.log('🔄 Parent clicked! Toggling submenu...');
            this.classList.toggle('open');
            subMenu.classList.toggle('open');
            console.log('📂 Submenu open:', subMenu.classList.contains('open'));
        });
        ...
        subLinks.forEach(link => {
            link.addEventListener('click', function(e) {
                ...
                console.log('🔗 Sub-link clicked:', targetId);
                if (targetElement) {
    ...
    mainLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            const targetId = this.getAttribute('href').substring(1);
            const targetElement = aboutPage.querySelector(`#${targetId}`);
            console.log('🔗 Main link clicked:', targetId);
            if (targetElement) {
                const targetTop = targetElement.offsetTop - 80;
                aboutPage.scrollTo({
    top: targetTop,
