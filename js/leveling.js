...
function levelUp(ellapsedTime = null) {
  ...
  if (data && data.objectFor === 'stopwatch' && ellapsedTime !== null) {
    ...
    let baseXp = studyTime * xpfactor;
    let basePp = studyTime * ppfactor;

    const xpBonus = data.xpBonus ?? 1;
    const ppBonus = data.ppBonus ?? 1;

    gainedXp = Math.round(baseXp * xpBonus);
    gainedPp = Math.round(basePp * ppBonus);
