...
function validateAch() {
  ...
  const validations = [
    ...
    [playerState.rewardStreak >= 4, "Yum...Piece Of Cake..", 7],
    ...
    [playerState.normalBossDefeated === true, "Normal Boss Defeated", 13],
    [playerState.eliteBossDefeated === true, "An Epic Boss Defeated", 14],
    [playerState.legenderyBossDefeated === true, "Woaaa... Legendery!!!", 15],
    [playerState.bossDefeatCount >= 15, "The King Of Blood🩸", 16]
  ];
...
function createTaskBox(data = null) {
  ...
  cancelBtn.addEventListener('click', cancelOngoingTask);
  taskBox.appendChild(cancelBtn);
  
  if (taskData.fromTodoList) {
      setTimeout(()=>{
...
function cancelOngoingTask() {
    const confirmation = confirm('Are You Sure You Want To Cancel The Ongoing Task?');
    if (confirmation) {
        ...
        const cancelBtn = taskBox.querySelector('.task-cancel-btn');
        cancelBtn.remove();
        ...
        if (sw && sw.linkedTodoId) {
            ...
            if (todoIndex !== -1) {
                todoArray[todoIndex].done = false;
