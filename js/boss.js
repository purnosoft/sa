...
function chooseBoss(player) {
    ...
    const canSpawnLegend = level >= BOSSES.legend.unlockAfter;
    const canSpawnElite  = level >= BOSSES.elite.unlockAfter;
    
    const legendChance = canSpawnLegend
        ? Math.min(player.bossKills.elite * 15 + level, 100)
        : 0;

    const eliteChance = canSpawnElite
        ? Math.min(player.bossKills.normal * 10 + level * 2, 100)
        : 0;

    if (canSpawnLegend && chance(legendChance)) {
        player.bossKills.elite = 0;
        return "legend";
    }

    if (canSpawnElite && chance(eliteChance)) {
        player.bossKills.normal = 0;
        return "elite";
    }

    if (level < BOSSES.normal.unlockAfter) {
        return null;
    }
