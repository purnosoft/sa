...
allButtons.forEach(btn => {
  btn.addEventListener('click', (e) => {
   ...
   if (clickedBtn.id === 'replayPageBtn') return;
   playSound('click');
...
function initializeData() {
  ...
  let appData = getLocalStorage('appData');
  if (!appData) {
    appData = { name: "Study-Assist", version: 2.2, lastActiveDay: null, lastTaskCompleted: null };
    setLocalStorage('appData', appData);
  }
  ...
  if (!playerState) {
      playerState = {
          ...
          normalBossDefeated: false,
          eliteBossDefeated: false,
          legenderyBossDefeated: false,
          bossDefeatCount: 0
